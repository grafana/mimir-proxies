...
find_last_release() {
  LAST_RELEASE_TAG=$(git tag --list --sort=taggerdate 'mimir-proxies-[0-9]*' | tail -1)
  ...
  # Find the last release version. Since all tags start with "mimir-proxies-"
  # we can simply find the version:
  LAST_RELEASE_VERSION=$(echo "${LAST_RELEASE_TAG}" | cut -c 15-)
...
find_prev_release() {
  ...
  if [[ $LAST_RELEASE_VERSION =~ "-rc" ]]; then
    PREV_RELEASE_TAG=$(git tag --list --sort=taggerdate 'mimir-proxies-[0-9]*' | tail -2 | head -1)
  else
    PREV_RELEASE_TAG=$(git tag --list --sort=taggerdate 'mimir-proxies-[0-9]*' | grep -v -- '-rc' | tail -2 | head -1)
  fi
  ...
  # Find the prev release version. Since all tags start with "mimir-proxies-"
  # we can simply find the version:
  PREV_RELEASE_VERSION=$(echo "${PREV_RELEASE_TAG}" | cut -c 15-)
